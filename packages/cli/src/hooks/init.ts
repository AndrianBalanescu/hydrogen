...
import {
  ...
  isHydrogenProject,
} from '../lib/hydrogen-primitives-command-policy.js';
...
const hook: Hook<'init'> = async function (options) {
  ...
  const hydrogenPrimitivesPolicyResult =
    await applyHydrogenPrimitivesCommandPolicy({
      id: options.id,
      projectPath,
    });

  if (hydrogenPrimitivesPolicyResult.action === 'exit') {
    process.exit(hydrogenPrimitivesPolicyResult.status);
  }
